...
class ApiAuth(object):
    ...
    def get_token(self, id=None, username=None,  password=None, designation=None):
        ...
        # print('生成token')
        token = str(id) + self.split_character + username + self.split_character + password + self.split_character+ designation
            ...
            def decorated(*args, **kwargs):
                ...
                if 'token' in request.headers:
                    ...
                    try:
                        ...
                        id, user, hash_password, designation = token.split(self.split_character)
                        # print('user：', user, 'hash_password', hash_password)
                        # 如果账号和密码都存在
                        if user and hash_password:
                            auth_user = {'id': id, 'user': user, 'pwd': hash_password}
                ...
                if auth_user:
                    ...
                    if self.verify_password_callback:
                        # print('开始验证密码')
                        user = self.verify_password_callback(auth_user.get('id'), auth_user.get('user'))
                        if user:
                            # print('密码验证成功')
                            # 如果user不为空，加载
    ...
    def decode(self, token=None):
        ...
        id, username, password, designation = info.split(' ')
        return id, username, designation
...
def verify_password(id, username):
    key = str(id) + '_USER_' + username.upper()
    data = get(key)
    if data:
        log.debug('信息验证成功!')
        return True
